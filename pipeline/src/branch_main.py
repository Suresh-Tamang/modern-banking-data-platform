...
from src.etl.transform.core import normalize_branches
...
def run(source: str, load_mode: str) -> None:
    ...
    if source == "api":
        ...
        try:
            rows = list(api.paginate(
                endpoint=cfg["sources"]["api"]["endpoint_branch"]))
    ...
    normalized = [c.model_dump() for c in normalize_branches(rows)]

    # load
    dsn = config.env("POSTGRES_DSN")
    target_table = cfg["run"]["branch_table"]
