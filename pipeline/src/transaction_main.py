...
from .etl.extract import transactions_extraction
from .etl.transform import transaction_transformation
from .etl.load import transaction_upsert
...
def run() -> None:
    ...
    transaction_api = transactions_extraction.TransactionClient(
    ...
    for record in transaction_api.get():
        rows.append(record)
    ...
    if df_transformed.empty:
        logger.info("No data to load after transformation. Exiting ETL process.")
        return  
    #load data
